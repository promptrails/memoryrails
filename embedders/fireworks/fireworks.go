...
const (
	defaultBaseURL = "https://api.fireworks.ai/inference/v1/embeddings"
	defaultModel   = "nomic-ai/nomic-embed-text-v1.5"
)

// Embedder generates embeddings using Fireworks AI's OpenAI-compatible API.
...
// WithModel sets the model. Default: nomic-ai/nomic-embed-text-v1.5.
func WithModel(model string) Option { return func(e *Embedder) { e.model = model } }

// WithDimensions sets the expected embedding dimensions. Default: 768.
...
// New creates a new Fireworks AI embedder.
...
func (e *Embedder) EmbedBatch(ctx context.Context, texts []string) ([][]float32, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("fireworks embedder: marshal error: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, e.baseURL, bytes.NewReader(reqBody))
	if err != nil {
		return nil, fmt.Errorf("fireworks embedder: request error: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("fireworks embedder: request failed: %w", err)
	}
	defer resp.Body.Close()

	body, _ := io.ReadAll(resp.Body)
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("fireworks embedder: API error (status %d): %s", resp.StatusCode, string(body))
	}
	...
	if err := json.Unmarshal(body, &result); err != nil {
		return nil, fmt.Errorf("fireworks embedder: parse error: %w", err)
	}
